...
def main():
  ...
  #step 1  将content_img指定input , 这里进行了第一次sess.run() ,将content图片的 conv4_2 层输出计算出来, 既 --> net[l[0]])
  ...
  #step 2 将style_img 指定input, 这里进行了第二次sess.run() ,将style图片的 conv1_1 ，conv2_1 ... 层的输出计算出来
  ...
  #step 4 将 noise_img 指定为input，然后进行了真正的计算
  # 计算的是noise这张图片和content 和style图片的cost ，然后使他们的cost越来越小，也就是不断的学习他们的特征
  sess.run(tf.initialize_all_variables())
  ...
  for i in range(ITERATION):
    print ("cost_content : ",sess.run(cost_content))
    sess.run(train)
    if i%100 ==0:
